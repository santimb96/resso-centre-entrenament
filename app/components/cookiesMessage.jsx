...
export default function CookiesMessage() {
  ...
  return (
    !cookiesAllowed && (
      <>
        <div className='z-50 border border-secondary fixed bottom-5 bg-primary shadow-custom rounded-md px-5 py-2.5 hidden md:flex md:w-[80%] lg:w-[70%] xl:w-[60%] justify-start items-center gap-2.5'>
        ...
        <button onClick={() => saveCookiesPreference(true)} className='z-50 fixed bottom-20 right-5 bg-primary shadow-custom rounded-full block md:hidden border border-secondary p-1'><Cookie size={36} /></button>
      </>
    )
  )
}
